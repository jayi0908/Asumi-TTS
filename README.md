...
本仓库只保留**服务**：调用包、模型、词典。训练侧的东西（数据清洗、VAD 切分、响度归一、
转写、SBV2 预处理、参数调优与评测）既不在服务路径上，也已经完成使命，因此不随仓库分发
—— 需要时从 git 历史里取。数据与中间产物（`processed/`、`build/`）同理：可从原始素材
重跑，且原始素材本身也不随仓库分发。要点：
